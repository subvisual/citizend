...
import {SaleTestCase} from "../../../contracts/token/SaleTestCase.sol";
...
contract SaleTestCase1 is Test {
    ...
    function test_case1() public {
        ...
        sale.buy(1000 ether, aliceMerkleProof);
        vm.stopPrank();

        vm.startPrank(bob);
        sale.buy(1000 ether, bobMerkleProof);
        vm.stopPrank();

        vm.startPrank(carol);
        sale.buy(1000 ether, carolMerkleProof);
        vm.stopPrank();

        vm.startPrank(dave);
        sale.buy(1000 ether, daveMerkleProof);
        vm.stopPrank();

        vm.startPrank(foo);
        sale.buy(1000 ether, fooMerkleProof);
        vm.stopPrank();
